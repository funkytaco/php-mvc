...
class ApplicationTasks {
    ...
    private static function showUsefulCommands(string $appName) {
        ...
        // Check if dns-setup-{appName}-hosts.sh exists
    ...
    /**
     * Explain the two places app credentials can live, and how to move them
     * into the vault. Shown whenever a vault command comes up empty, so the
     * user learns their options instead of hitting a dead end.
     */
    private static function showCredentialStorageHelp(?string $appName = null): void
    {
        $app = $appName ?? '<app>';
        echo PHP_EOL;
        echo self::ansiFormat('INFO', '💡 Where credentials live:');
        echo "  1. Live copy (always): .installer/apps/$app/app.nimbus.json — plaintext, gitignored." . PHP_EOL;
        echo "     View with: composer nimbus:view $app" . PHP_EOL;
        echo "  2. Vault (optional): encrypted backup via ansible-vault, survives app re-creation." . PHP_EOL;
        echo "     composer nimbus:vault-init          # one-time vault setup" . PHP_EOL;
    ...
    public static function nimbusVaultView(Event $event): void
    {
        ...
        try {
            ...
            if (!$vaultManager->isInitialized()) {
                echo self::ansiFormat('ERROR', 'Vault not initialized. Run: composer nimbus:vault-init');
                self::showCredentialStorageHelp($specificApp);
                return;
            }
            ...
            if ($specificApp) {
                if (!isset($allCredentials['apps'][$specificApp])) {
                    echo self::ansiFormat('ERROR', "No credentials found in the vault for app: $specificApp");
                    self::showCredentialStorageHelp($specificApp);
                    return;
                }
            ...
            echo self::ansiFormat('INFO', 'ℹ️  These are the encrypted vault backups. The live values apps actually run with are in .installer/apps/<app>/app.nimbus.json (plaintext, gitignored).');
            if (!$specificApp) {
